...
class Animator extends DisplayObjectContainer{
  ...
  load() async {
    // Path to JSON file
    _resources.addTextFile(_animation_path, _animation_path);
    await _resources.load();
    String json = _resources.getTextFile(_animation_path);
    List animations = JSON.decode(json);

    for (Map animation in animations) {
      if (!_resources.containsBitmapData(animation['image'])) {
        _resources.addBitmapData(animation['image'], animation['image']);
        await _resources.load();
      }

      BitmapData bitmapData = _resources.getBitmapData(animation['image']);
